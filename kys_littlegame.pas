...
begin
  ...
  begin
    grp := FileOpen(GAME_file, fmopenread);
  ...
  begin
    ...
    if (integer(now) - integer(ori_time) >= delay) then
    begin
      ori_time := SDL_GetTicks;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        arrowdegree := showarrow(arrowpic, event.button.x, event.button.y, arrowstep, arrowdegree);
    ...
    showbow(bowpic[readystate], event.button.x, event.button.y, degree);
    word := '得分：';
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            degree := showbow(bowpic[readystate], event.button.x, event.button.y, degree);
            //  showbow(Gamepic,bowpic[readystate], event.button.x, event.button.y,readystate);
      ...
      begin
        if event.button.button = SDL_BUTTON_LEFT then
          ...
          begin
            ...
            Dec(chance);
            arrowstep := arrowstep + arrowspeed;
            degree := showbow(bowpic[readystate], event.button.x, event.button.y, 180);
            //      showbow(Gamepic,bowpic[readystate], event.button.x, event.button.y,readystate);
          end;
      end;
      SDL_MOUSEBUTTONDOWN:
...
begin
  ...
  //read file from bin
  col := FileOpen('list\Acupuncture.bin', fmopenread);
    ...
    begin
      ...
      begin
        ...
        begin
          for i := 0 to acunum - 1 do
          begin
            if (event.button.x <= (AcupunctureList[2 * (i + 3)] + accu)) and
              (event.button.x >= (AcupunctureList[2 * (i + 3)] - accu)) and
              (event.button.y >= (AcupunctureList[2 * (i + 3) + 1] - accu)) and
              (event.button.y <= (AcupunctureList[2 * (i + 3) + 1] + accu)) then
            begin
              select[s] := i;
              s := s + 1;
              drawpngpic(gamepic,
...
begin
  ...
  begin
    ...
    case event.type_ of
      SDL_MOUSEMOTION:
      begin
        if (event.button.x > x) and (event.button.x < x + 50 * c) and (event.button.y > y) and
          (event.button.y < y + 50 * r) then

          menu := ((event.button.x - x) div 50) + (((event.button.y - y) div 50) * c);
      end;
      SDL_MOUSEBUTTONUP:
      begin
        if event.button.button = SDL_BUTTON_LEFT then
...
begin
  ...
  begin
    ...
    case event.type_ of
      SDL_MOUSEMOTION:
      begin
        if (event.button.x > (wx1 - 11)) and (event.button.x < (wx1 - 11) + 40 * c) and
          (event.button.y > (wy1 - 9)) and (event.button.y < (wy1 - 9) + 40 * r1) then

          menu1 := ((event.button.x - wx1 - 11) div 40) + (((event.button.y - wy1 + 9)) div 40) * c;
      end;
      SDL_MOUSEBUTTONUP:
      begin
        if event.button.button = SDL_BUTTON_LEFT then
...
begin
  ...
  begin
    ...
    case event.type_ of
      ...
      SDL_MOUSEMOTION:
      begin
        if (event.button.x > (wx - 11)) and (event.button.x < (wx - 11) + 40 * c) and
          (event.button.y > (wy - 9)) and (event.button.y < (wy - 9) + 40 * row) then
          menu := ((event.button.x - wx - 11) div 40) + (((event.button.y - wy + 9)) div 40) * c;
      end;
      SDL_MOUSEBUTTONUP:
        if event.button.button = SDL_BUTTON_LEFT then
        begin
          if (event.button.x > (wx - 11)) and (event.button.x < (wx - 11) + 40 * c) and
            (event.button.y > (wy - 9)) and (event.button.y < (wy - 9) + 40 * r1) then
            menu := ((event.button.x - wx - 11) div 40) + (((event.button.y - wy + 9)) div 40) * c;
          chance := SelectPoetry(wx1, wy1, (Count div r1), Count, len, menu, chance);
          SDL_UpdateRect2(screen, 0, 0, 640, 440);
        end;
  ...
  begin
    ...
    case event.type_ of
      ...
      begin
        ...
        if (event.button.x > x) and (event.button.x < x - 5 + w) and (event.button.y > y) and
          (event.button.y < y - 5 + h) then
        begin
          menu := ((event.button.x - x) div 80) + ((event.button.y - y - 30) div 80) * 5;
