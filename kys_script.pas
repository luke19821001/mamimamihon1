...
begin
  ...
  begin
    h := FileOpen(filename1, fmopenread);
    len := FileSeek(h, 0, 2);
...
begin
  ...
  begin
    ...
    case t of
      ...
      2: DrawBPic(p, x, y, 0);
      3: {DrawHeadPic(p, x, y);} ZoomPic(head_pic[p].pic, 0, x, y - 60, 58, 60);
      // 4: DrawEPic(p, x, y);
      //5:
    end;
  end;
...
begin
  if floor(lua_tonumber(L, -1)) = 0 then
    SelectAim(floor(lua_tonumber(L, -3)), floor(lua_tonumber(L, -2)),0);
...
begin
  ...
  begin
    ...
    begin
      ...
      ZoomPic(head_pic[headnum].pic, 0, hx, hy + 57 - 60, 58, 60);
    end;
    if namenum <> 0 then
    begin
