...
procedure Savescreen(num:integer); overload;
procedure Savescreen(num,x,y,w,h:integer;Scr:PSDL_Surface); overload;
//重画一个屏幕
procedure Reshowscreen(num:integer); overload;
procedure Reshowscreen(num,x,y,w,h:integer); overload;
//模擬單挑
function AIVS(rnum1,rnum2:smallint):boolean;

//作弊的後果
procedure dealcheat;

procedure CheckLoadCheat;
...
begin
  Result := False;
  t:=TimeToNum;
  if (DData[snum, eventnum, 11] >= 0) and (t >= DData[snum, eventnum, 11]) and
  ...
  begin
    ...
    begin
      ...
      begin
        if ((t - DData[snum, eventnum, 11]) mod DData[snum, eventnum, 13]) < DData[snum, eventnum, 12] then
          Result := True;
      end
      else
      if (t - DData[snum, eventnum, 11]) < DData[snum, eventnum, 12] then Result := True;
    end;
...
begin
  ...
  for i := 0 to length(DData) - 1 do
    ...
    begin
      ...
      if (DData[i, j, 0] > 9) and (DData[i, j, 11] >= 0) and (t >= DData[i, j, 11]) and
        (DData[i, j, 12] > 0) and (DData[i, j, 15] = 0) then
      begin
        if DData[i, j, 13] > 0 then
        begin
          if ((t - DData[i, j, 11]) mod DData[i, j, 13]) = DData[i, j, 12] then
          begin
            Rrole[DData[i, j, 0] div 10].MRevent := -1;
          end;
        end
        else if (t - DData[i, j, 11]) = DData[i, j, 12] then
        begin
          Rrole[DData[i, j, 0] div 10].MRevent := -1;
        end;
      end;
  ...
  begin
    if (TimeTrigger.adds[i].types = 0) and (TimeTrigger.adds[i].stime + TimeTrigger.adds[i].Dtime <= t) then
    begin
      deltimetrigger(i);
    end
    else if (TimeTrigger.adds[i].types = 1) and ((TimeTrigger.adds[i].endtime = 0) or (t < TimeTrigger.adds[i].endtime)) and (t = TimeTrigger.adds[i].stime) then
    begin
      callevent(TimeTrigger.adds[i].event);
    end;
  end;
end;
...
var
  ...
  zhiwujc9, mnum, nolw, lwstatus, nnum, lwnum, p, mexp,tmp0: integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Rrole_a[i].exp := Rrole_a[i].exp + Rrole[i].exp - tmp0;
        ...
        Rrole_a[i].ExpForBook:= Rrole_a[i].ExpForBook + Rrole[i].ExpForBook -tmp0;
        while (Rrole[i].Level < MAX_LEVEL) and (uint16(Rrole[i].Exp) >= uint16(LevelUplist[Rrole[i].Level - 1])) do
        begin
          Rrole_a[i].Exp := Rrole_a[i].Exp - LevelUplist[Rrole[i].Level - 1];
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                EatOneItem(i, Rmagic[mnum].miji, False);
                tmp0 := Rrole[i].lzhaoshi[tmp[tm]];
                Rrole[i].lzhaoshi[tmp[tm]] := Rrole[i].lzhaoshi[tmp[tm]] or (1 shl j2);
                Rrole_a[i].lzhaoshi[tmp[tm]] := Rrole_a[i].lzhaoshi[tmp[tm]] + Rrole[i].lzhaoshi[tmp[tm]] - tmp0;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  Rrole_a[i].lmagic[k1] := Rrole_a[i].lmagic[k1] + Rmpmagic[mpnum][k] - Rrole[i].lmagic[k1];
    ...
    begin
      ...
      begin
        ...
        begin
          if (random(100) > Rrole[i].msq) and (random(Rmenpai[mpnum].dizi) > 7) then
            continue;
          tm1 := random(n1);
          tmp0:=Rrole[i].maglevel[tmp1[tm1]];
          ...
          Rrole_a[i].maglevel[tmp1[tm1]]:=Rrole_a[i].maglevel[tmp1[tm1]]+Rrole[i].maglevel[tmp1[tm1]]-tmp0;
      ...
      begin
        ...
        begin
          ...
          tm2 := random(n2);
          tmp0:=Rrole[i].maglevel[tmp2[tm2]];
          ...
          Rrole_a[i].maglevel[tmp2[tm2]]:=Rrole_a[i].maglevel[tmp2[tm2]]+Rrole[i].maglevel[tmp2[tm2]]-tmp0;
...
begin
  ...
  if flag then
  begin
    Rrole_a[num].lmagic[k] := Rrole_a[num].lmagic[k] + j - Rrole[num].lmagic[k];
...
var
  ...
  str: WideString;
begin
  mag1:=GetGeliveAblemag(rnum1);
  mag2:=GetGeliveAblemag(rnum2);
  result:=false;
  while (true)do
  begin
    if mag1 >= 0 then
    begin
      HurtValueAI(rnum1, rnum2,Rrole[rnum1].lmagic[mag1],Rrole[rnum1].maglevel[mag1] div 100);
      if Rrole[rnum2].CurrentHP <= 0 then
      begin
        result:=true;
        exit;
      end;
      HurtValueAI(rnum2, rnum1,Rrole[rnum2].lmagic[mag2],Rrole[rnum2].maglevel[mag2] div 100);
      if Rrole[rnum1].CurrentHP <= 0 then
      begin
        result:=false;
        exit;
      end;
...
procedure HurtValueAI(Rnum1, Rnum2, mnum, level: integer);
var
  i, l1, c, mhurt, p, def, mp1, mp2, addatt, att, spd2, wpn2, spd1, wpn1, k1, k2, knowledge, dis, res,tmp: integer;
  a1, s1, m1, w1: double;
begin
  ...
  if (res > 9999) then
    res := 9999;
  tmp:= Rrole[rnum1].CurrentMP;
  Rrole[rnum1].CurrentMP := max(0, Rrole[rnum1].CurrentMP - level * Rmagic[mnum].NeedMP);
  Rrole_a[rnum1].CurrentMP:=Rrole_a[rnum1].CurrentMP + Rrole[rnum1].CurrentMP - tmp;
  tmp:=Rrole[rnum2].CurrentHP;
  Rrole[rnum2].CurrentHP := max(0, Rrole[rnum2].CurrentHP - res);
  Rrole_a[rnum2].CurrentHP:=Rrole_a[rnum2].CurrentHP + Rrole[rnum2].CurrentHP - tmp;
end;
...
begin
  ...
  begin
    ...
    Rrole_a[i].CurrentHP:=Rrole_a[i].CurrentHP + Rrole[i].CurrentHP - tmp;
    tmp:=Rrole[i].CurrentMP;
    ...
    Rrole_a[i].CurrentMP:=Rrole_a[i].CurrentMP + Rrole[i].CurrentMP -tmp;

    tmp:=Rrole[i].PhyPower;
    Rrole[i].PhyPower := max(1, min(100, Rrole[i].PhyPower + 3 - (Rrole[i].Hurt + Rrole[i].Poision) div 50));
    Rrole_a[i].PhyPower:=Rrole_a[i].PhyPower + Rrole[i].PhyPower - tmp;
    ...
    Rrole_a[i].Hurt:=Rrole_a[i].Hurt + Rrole[i].Hurt -tmp;
  end;
end;
//讀取戰場語錄
...
begin
  dest.x := x;
  dest.y := y;
  dest.w := min(w,Scr.w - dest.x);
  dest.h := min(h,Scr.h - dest.y);
  if (Scr.w = screens[num].w) and (Scr.h = screens[num].h) then
  begin

    SDL_BlitSurface(Scr, @dest, Screens[num], @dest);

  end;
end;
procedure savescreen(num:integer); overload;
begin
  savescreen(num, 0, 0, screens[num].w, screens[num].h,preScreen);
end;
//重现一个屏幕
procedure reshowscreen(num:integer); overload;
begin
  reshowscreen(num, 0, 0, screens[num].w, screens[num].h);
end;
procedure reshowscreen(num,x,y,w,h:integer); overload;
var
  dest: TSDL_Rect;
begin
  dest.x := x;
  dest.y := y;
  dest.w := min(w,Screen.w - dest.x);
  dest.h := min(h,Screen.h - dest.y);
  if (Screen.w = screens[num].w) and (Screen.h = screens[num].h) then
  begin

    SDL_BlitSurface(Screens[num], @dest, Screen, @dest);

  end;
end;
procedure dealcheat;
begin
  MessageBox(0, '修改游戏会降低游戏乐趣，请不要修改！', '防修改', MB_OK);
end;
procedure CheckLoadCheat;
var
  ...
  idx, grp, i1,i2, len, len1, wei,count: integer;
  RoleOffset: integer;
  tmp:uint16;
  savedata:Tsave;
  ischeat:boolean;
...
begin
  count:=0;
  filename := 'R' + IntToStr(savenum);
  filename1 := 'save\' + 'R' + IntToStr(savenum) + '.grp';
  ...
  setlength(Ritemlist_s,MAX_ITEM_AMOUNT);
  CopyMemory(@Ritemlist_s[0],@savedata.a[wei],len1);
  wei:=wei + len1;
  len1:=len - RoleOffset;
  setlength(Rrole_s, len1 div sizeof(Trole));
  CopyMemory(@Rrole_s[0],@savedata.a[wei],len1);
  ischeat:=false;
  for i1 :=0 to length(Ritemlist_s) - 1 do
  begin
    if ((Ritemlist[i1].Number - Ritemlist_s[i1].Number) <> Ritemlist_a[i1].Number)then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='物品编号：' + inttostr(i1);
      ischeat:=true;
    end;
    if((Ritemlist[i1].Amount - Ritemlist_s[i1].Amount) <> Ritemlist_a[i1].Amount) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='物品数量：'+ inttostr(i1);
      ischeat:=true;
    end;
  end;
  for i1:=0 to length(Rrole) - 1 do
  begin
    if ((Rrole[i1].PracticeBook - Rrole_s[i1].PracticeBook) <> Rrole_a[i1].PracticeBook) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='PracticeBook：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].menpai - Rrole_s[i1].menpai) <> Rrole_a[i1].menpai) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='menpai：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Defence - Rrole_s[i1].Defence) <> Rrole_a[i1].Defence) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Defence：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Speed - Rrole_s[i1].Speed) <> Rrole_a[i1].Speed) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Speed：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Attack - Rrole_s[i1].Attack) <> Rrole_a[i1].Attack) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Attack：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].IncLife - Rrole_s[i1].IncLife) <> Rrole_a[i1].IncLife) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='IncLife：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].MPType - Rrole_s[i1].MPType) <> Rrole_a[i1].MPType) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='MPType：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].CurrentMP - Rrole_s[i1].CurrentMP) <> Rrole_a[i1].CurrentMP) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='CurrentMP：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].MaxMP - Rrole_s[i1].MaxMP) <> Rrole_a[i1].MaxMP) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='MaxMP：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].CurrentHP - Rrole_s[i1].CurrentHP) <> Rrole_a[i1].CurrentHP) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='CurrentHP：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].MaxHP - Rrole_s[i1].MaxHP) <> Rrole_a[i1].MaxHP) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='MaxHP：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Unusual - Rrole_s[i1].Unusual) <> Rrole_a[i1].Unusual) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Unusual：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Knife - Rrole_s[i1].Knife) <> Rrole_a[i1].Knife) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Knife：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Sword - Rrole_s[i1].Sword) <> Rrole_a[i1].Sword) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Sword：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Fist - Rrole_s[i1].Fist) <> Rrole_a[i1].Fist) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Fist：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].UsePoi - Rrole_s[i1].UsePoi) <> Rrole_a[i1].UsePoi) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='UsePoi：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Medcine - Rrole_s[i1].Medcine) <> Rrole_a[i1].Medcine) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Medcine：'+ inttostr(i1);
      ischeat:=true;
    end;
    for i2:=0 to 29 do
    begin
      if ((Rrole[i1].LMagic[i2] - Rrole_s[i1].LMagic[i2]) <> Rrole_a[i1].LMagic[i2]) then
      begin
        inc(count);
        setlength(note,count);
        note[count - 1]:='LMagic：'+ inttostr(i1) + ' ' + inttostr(i2);
        ischeat:=true;
      end;
    end;
    for i2:=0 to 29 do
    begin
      if ((Rrole[i1].MagLevel[i2] - Rrole_s[i1].MagLevel[i2]) <> Rrole_a[i1].MagLevel[i2]) then
      begin
        inc(count);
        setlength(note,count);
        note[count - 1]:='MagLevel：'+ inttostr(i1) + ' '+inttostr(i2);
        ischeat:=true;
      end;
    end;
    for i2:=0 to 29 do
    begin
      if ((Rrole[i1].LZhaoshi[i2] - Rrole_s[i1].LZhaoshi[i2]) <> Rrole_a[i1].LZhaoshi[i2]) then
      begin
        inc(count);
        setlength(note,count);
        note[count - 1]:='LZhaoshi：'+ inttostr(i1) + ' ' + inttostr(i2);
        ischeat:=true;
      end;
    end;

    if ((Rrole[i1].fuyuan - Rrole_s[i1].fuyuan) <> Rrole_a[i1].fuyuan) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='fuyuan：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Aptitude - Rrole_s[i1].Aptitude) <> Rrole_a[i1].Aptitude) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Aptitude：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].xiangxing - Rrole_s[i1].xiangxing) <> Rrole_a[i1].xiangxing) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='xiangxing：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].HidWeapon - Rrole_s[i1].HidWeapon) <> Rrole_a[i1].HidWeapon) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='HidWeapon：'+ inttostr(i1);
      ischeat:=true;
    end;
    tmp:=Rrole[i1].Exp - Rrole_s[i1].Exp;
    if (tmp <> Rrole_a[i1].Exp) then
    begin
      inc(count);
      setlength(note,count);
      writeln(inttostr(tmp));
      note[count - 1]:='Exp：'+ inttostr(i1);
      ischeat:=true;
    end;
    tmp:=Rrole[i1].ExpForBook - Rrole_s[i1].ExpForBook;
    if (tmp<> Rrole_a[i1].ExpForBook) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='ExpForBook：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Hurt - Rrole_s[i1].Hurt) <> Rrole_a[i1].Hurt) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Hurt：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Poision - Rrole_s[i1].Poision) <> Rrole_a[i1].Poision) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Poision：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].PhyPower - Rrole_s[i1].PhyPower) <> Rrole_a[i1].PhyPower) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='PhyPower：'+ inttostr(i1);
      ischeat:=true;
    end;
    for i2:=0 to 4 do
    begin
      if ((Rrole[i1].Equip[i2] - Rrole_s[i1].Equip[i2]) <> Rrole_a[i1].Equip[i2]) then
      begin
        inc(count);
        setlength(note,count);
        note[count - 1]:='Equip：'+ inttostr(i1) + ' ' + inttostr(i2);
        ischeat:=true;
      end;
    end;
    if ((Rrole[i1].Angry - Rrole_s[i1].Angry) <> Rrole_a[i1].Angry) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Angry：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].rehurt - Rrole_s[i1].rehurt) <> Rrole_a[i1].rehurt) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='rehurt：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].MedPoi - Rrole_s[i1].MedPoi) <> Rrole_a[i1].MedPoi) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='MedPoi：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].DefPoi - Rrole_s[i1].DefPoi) <> Rrole_a[i1].DefPoi) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='DefPoi：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Knowledge - Rrole_s[i1].Knowledge) <> Rrole_a[i1].Knowledge) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Knowledge：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Ethics - Rrole_s[i1].Ethics) <> Rrole_a[i1].Ethics) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Ethics：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].AttPoi - Rrole_s[i1].AttPoi) <> Rrole_a[i1].AttPoi) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='AttPoi：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].AttTwice - Rrole_s[i1].AttTwice) <> Rrole_a[i1].AttTwice) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='AttTwice：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].Repute - Rrole_s[i1].Repute) <> Rrole_a[i1].Repute) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='Repute：'+ inttostr(i1);
      ischeat:=true;
    end;
    if ((Rrole[i1].jiaoqing - Rrole_s[i1].jiaoqing) <> Rrole_a[i1].jiaoqing) then
    begin
      inc(count);
      setlength(note,count);
      note[count - 1]:='jiaoqing：'+ inttostr(i1);
      ischeat:=true;
    end;
  end;
  if ischeat then
